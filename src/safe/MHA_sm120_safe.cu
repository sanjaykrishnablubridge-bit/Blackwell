...
__global__ void mha_v3_tc(
    ...
    __shared__ float Qs_hi [Br][D];
    __shared__ float Qs_lo [Br][D];
    __shared__ float Ks_hi [Bc][D];
    __shared__ float Ks_lo [Bc][D];
    __shared__ float Vs_hi [Bc][D];
    __shared__ float Vs_lo [Bc][D];
    __shared__ float Ss_hi [Br][Bc];
    __shared__ float Ss_lo [Br][Bc];
    ...
    for (int kt = 0; kt < num_ktiles; kt++) {
        ...
        for (int kk = 0; kk < D; kk += 8) {
            ...
            for (int n = 0; n < Bc / 16; n++) {
                ...
                // 3 mmas — recover full FP32 (drop a_lo*b_lo; it's 2^-20 and
                // doesn't move the needle: 4-mma tested gives identical max_abs).
        ...
        for (int r = 0; r < Br; r++) {
            ...
            for (int c = 0; c < kColsPerLane; c++)
                vv[c] = Ss_hi[r][lane * kColsPerLane + c] * softmax_scale;
            ...
            // Broadcast row r's running state from lane r to all lanes
            ...
            float scale_old = exp2_safe((m_old - m_new) * kLog2e);

            float pp[kColsPerLane];
            #pragma unroll
            for (int c = 0; c < kColsPerLane; c++)
                pp[c] = exp2_safe((vv[c] - m_new) * kLog2e);
...
int main() {
    ...
    printf("Launching mha_v3_tc  grid=(%d,%d)  block=(32)\n", S / Br, B * H);
    mha_v3_tc<Br, Bc, D><<<grid, block>>>(dQ, dK, dV, dO, scale, S);
